...
diop_list = [26,26.5,27,27.5,28,28.5,29]
diop_list = [24,25,26,27,28,29,30]
...
def tomar_foto(nombre):
    ...
    with Camera() as cam:
        ...
        cam.start()
        imgs = [cam.get_array().astype(float) for n in range(N_samples)]
        img = (sum(imgs)/N_samples).astype(np.uint16)
        cam.stop()
...
if __name__ == "__main__":
    ...
    with SSHConnection(RPI_IP, RPI_PORT, RPI_USER, RPI_PASS) as ssh:
        ...
        movimientos = [(45, 45), (45, 45), (45, 0), (135, 90)]
        orientaciones = [('a','h'), ('a','h'), ('a','h'), ('h','a')]

        # Recorrer filtros manualmente
        for filtro in filtros:

            print("\n--------------------------------")
            print(f"Colocar filtro: {filtro}")

            input("Presione ENTER cuando el filtro esté colocado...")

            for label, (m1, m2), (o1, o2) in zip(polar_labels, movimientos, orientaciones):
                ...
                for diop in diop_list:
                    ...
                    folder_corriente = os.path.join(output_path, folder_timestamp, f"{diop}", filtro)
